...
/* SELECT EMPREGADO */
SELECT nome, SUBSTRING(CAST(rg AS VARCHAR), 1,2) +'.' + SUBSTRING(CAST(rg AS VARCHAR), 3,3) + '.' +  SUBSTRING(CAST(rg AS VARCHAR), 6,8) AS RG, SUBSTRING(CAST(cic AS VARCHAR), 1,3) + '.' + SUBSTRING(CAST(cic AS VARCHAR), 3,3) + '-' + SUBSTRING(CAST(cic AS VARCHAR), 7,8) AS CPF  from tbl_empregado ORDER BY cic ASC
